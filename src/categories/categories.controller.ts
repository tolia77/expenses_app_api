...
export class CategoriesController {
    ...
    @Post()
    async create(@Body() categoryCreateDto: CategoryCreateDto, @CurrentUser() user) {
        return this.categoriesService.create(user.sub, categoryCreateDto);
    }

    @Patch(':id')
    async update(@Param('id') id: string, @Body() categoryUpdateDto: CategoryUpdateDto, @CurrentUser() user) {
